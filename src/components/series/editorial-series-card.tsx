...
export function EditorialSeriesCard({ series, currentUserId, showManagerName, demo }: EditorialSeriesCardProps) {
  ...
  return (
    <div className={`group relative bg-card rounded-2xl p-6 flex flex-col h-full overflow-hidden border border-[var(--editorial-outline-variant,var(--border))]/50 shadow-[0_1px_3px_rgba(0,0,0,0.02)] transition-all duration-300 hover:shadow-[0_10px_25px_-5px_rgba(0,0,0,0.05)] hover:-translate-y-0.5 hover:border-primary/40 ${sentimentClass} ${isOverdue ? "border-l-4 border-l-[var(--color-warning,#f59e0b)]" : ""}`}>
          ...
          {series.status === "active" && (
            <Tooltip>
              <TooltipTrigger asChild>
                <button
                  type="button"
                  className="pointer-events-auto relative z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-muted-foreground hover:text-primary hover:bg-accent transition-all"
      ...
      <div className="flex items-end justify-between pt-4 border-t border-[var(--editorial-outline-variant,var(--border))]/50 relative z-[1] pointer-events-none">
        <Tooltip>
          <TooltipTrigger asChild>
            <div className="pointer-events-auto flex flex-col gap-0.5 cursor-default">
        ...
        {isManager && series.status === "active" ? (
          <button
            ...
            className="pointer-events-auto relative z-10 flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-bold text-primary hover:bg-primary/5 transition-all active:scale-95 disabled:opacity-50"
          >
            {isInProgress ? (
          ...
          <span className="pointer-events-none text-xs font-bold text-primary opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1 relative z-10">
