...
import { X, CalendarDays, Repeat, Play, PlusCircle, GripVertical, AlertCircle, Sparkles } from "lucide-react";
        ...
        <header className="p-8 pb-6 border-b border-[var(--editorial-outline-variant,var(--border))]/10">
          <div className="flex justify-between items-start mb-6">
            <div className="flex items-center gap-4">
              <Avatar className="h-14 w-14 rounded-2xl ring-2 ring-primary/5">
                <ThemedAvatarImage name={personName} uploadedUrl={personAvatarUrl} role={personLevel} className="rounded-2xl" />
                <AvatarFallback className="text-sm font-bold rounded-2xl">{initials}</AvatarFallback>
              </Avatar>
              <div>
                <h2 className="text-xl font-headline font-extrabold text-foreground">{personName}</h2>
                {personJobTitle && (
                  <p className="text-sm text-muted-foreground font-medium">{personJobTitle}</p>
                )}
              </div>
            </div>
            <button
          ...
          <div className="flex flex-wrap gap-3 items-center">
            <span className={cn(
              ...
              {isOverdue ? t("editorial.overdue") : t("agenda.upcoming")}
            </span>
