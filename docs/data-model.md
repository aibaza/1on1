...
The schema has **25 tables** across 16 schema files in `src/lib/db/schema/`.
...
TENANT ─────────────────────────────────────────────────────────────┐
  ...
  │     ├── TEMPLATE_SECTION                                         │
  │     │     └── TEMPLATE_QUESTION                                  │
  │     └── TEMPLATE_LABEL_ASSIGNMENT ──► TEMPLATE_LABEL            │
  │                                                                  │
  ├── ANALYTICS_SNAPSHOT                                             │
...
Auth.js adapter tables (no tenant_id):
...
| `ai_chat_history` | `JSONB` | nullable | Persisted AI editor chat messages |
| `ai_version_history` | `JSONB` | nullable | Snapshots per AI turn (for revert) |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |
| `updated_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |
...
| `section_id` | `UUID` | FK → TEMPLATE_SECTION, NOT NULL | Which wizard step this question belongs to |
...
| `sort_order` | `INTEGER` | NOT NULL | Display order within section |
| `score_weight` | `DECIMAL(4,2)` | NOT NULL, default 1 | Weight for session score computation |
...
User-defined groups within a template that become wizard steps.

| Column | Type | Constraints | Description |
|--------|------|------------|-------------|
| `id` | `UUID` | PK, default gen_random_uuid() | |
| `template_id` | `UUID` | FK → QUESTIONNAIRE_TEMPLATE, NOT NULL | |
| `tenant_id` | `UUID` | FK → TENANT, nullable | Denormalized for RLS |
| `name` | `VARCHAR(255)` | NOT NULL | Section display name |
...
Tenant-scoped taxonomy tags for organizing templates.

| Column | Type | Constraints | Description |
|--------|------|------------|-------------|
| `id` | `UUID` | PK, default gen_random_uuid() | |
| `tenant_id` | `UUID` | FK → TENANT, NOT NULL | |
| `name` | `VARCHAR(100)` | NOT NULL | Label display name |
| `color` | `VARCHAR(7)` | | Hex color code (e.g., "#4F46E5") |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |

**Indexes:**
- `UNIQUE(tenant_id, name)` — label names unique within a tenant

---

### TEMPLATE_LABEL_ASSIGNMENT

Many-to-many join between templates and labels.

| Column | Type | Constraints | Description |
|--------|------|------------|-------------|
| `template_id` | `UUID` | FK → QUESTIONNAIRE_TEMPLATE, NOT NULL | |
| `label_id` | `UUID` | FK → TEMPLATE_LABEL, NOT NULL, ON DELETE CASCADE | |
...
Pre-session coaching nudges generated by the AI pipeline for a meeting series.
...
| `series_id` | `UUID` | FK → MEETING_SERIES, NOT NULL | |
| `tenant_id` | `UUID` | FK → TENANT, NOT NULL | |
| `target_session_at` | `TIMESTAMPTZ` | nullable | Session this nudge targets |
| `content` | `TEXT` | NOT NULL | Coaching suggestion text |
| `reason` | `TEXT` | | Why this nudge was generated |
| `priority` | `TEXT` | default 'medium' | `'high'`, `'medium'`, or `'low'` |
| `source_session_id` | `UUID` | FK → SESSION, nullable | Session that triggered this nudge |
| `is_dismissed` | `BOOLEAN` | NOT NULL, default false | Dismissed by the manager |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |
| `refreshed_at` | `TIMESTAMPTZ` | nullable | When the nudge was last regenerated |

**Indexes:**
- `INDEX(series_id, target_session_at)` — fetch nudges for upcoming sessions
- `INDEX(tenant_id, is_dismissed)` — filter active nudges
...
| `id` | `UUID` | PK, default gen_random_uuid() | |
| `tenant_id` | `UUID` | FK → TENANT, NOT NULL | |
| `actor_id` | `UUID` | FK → USER, nullable | Who performed the action (NULL = system) |
| `action` | `VARCHAR(100)` | NOT NULL | e.g., `session.completed`, `user.invited` |
| `resource_type` | `VARCHAR(50)` | NOT NULL | e.g., `session`, `template`, `user` |
| `resource_id` | `UUID` | nullable | ID of the affected entity |
| `metadata` | `JSONB` | NOT NULL, default '{}' | Additional context (diff, params, etc.) |
| `ip_address` | `VARCHAR(45)` | | Actor's IP (IPv6 compatible) |
| `user_agent` | `TEXT` | | Browser/client user agent |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |

**Indexes:**
- `INDEX(tenant_id, action, created_at)` — audit log filtering by action type
- `INDEX(tenant_id, actor_id, created_at)` — audit log filtering by actor

---

### ACCOUNT

OAuth provider account links, managed by the Auth.js adapter. One user can have multiple OAuth accounts.

| Column | Type | Constraints | Description |
|--------|------|------------|-------------|
| `user_id` | `UUID` | FK → USER, NOT NULL, ON DELETE CASCADE | |
| `type` | `VARCHAR(255)` | NOT NULL | e.g., `oauth`, `email` |
| `provider` | `VARCHAR(255)` | NOT NULL | e.g., `google`, `microsoft` |
...
Database sessions for Auth.js adapter. Named `auth_session` to avoid collision with the app `session` table.
...
Magic link tokens for email-based sign-in, managed by the Auth.js adapter.

| Column | Type | Constraints | Description |
|--------|------|------------|-------------|
| `identifier` | `VARCHAR(255)` | NOT NULL | Email address |
| `token` | `VARCHAR(255)` | NOT NULL | One-time token |
...
Custom email verification tokens sent when users register with credentials (24h expiry).
...
| `expires_at` | `TIMESTAMPTZ` | NOT NULL | Token expiry (24h after creation) |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |
...
Tokens for the "forgot password" flow (1h expiry).
...
| `user_id` | `UUID` | FK → USER, NOT NULL, ON DELETE CASCADE | |
| `token` | `VARCHAR(255)` | UNIQUE, NOT NULL | Secure random token |
| `expires_at` | `TIMESTAMPTZ` | NOT NULL | Token expiry (1h after creation) |
| `created_at` | `TIMESTAMPTZ` | NOT NULL, default now() | |
...
Invite links generated by admins to onboard new users (7-day expiry).
...
| `role` | `ENUM('admin','manager','member')` | NOT NULL, default 'member' | Role to assign on acceptance |
| `token` | `VARCHAR(255)` | UNIQUE, NOT NULL | Secure random token |
| `invited_by` | `UUID` | FK → USER, NOT NULL | Admin who sent the invite |
